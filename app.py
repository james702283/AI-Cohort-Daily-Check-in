# app.py
# A simple web application using the Flask framework to serve the AI Cohort Check-in page.
...
# This is the HTML content for our web page.
# In a larger application, this would be in a separate file (e.g., templates/index.html).
# For a simple, single-file repository, embedding it here makes it easy to share and run.
INDEX_HTML = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        body {
            ...
            background-image: url('https://images.pexels.com/photos/1181359/pexels-photo-1181359.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2');
        ...
        .dark-theme-text label, .dark-theme-text h2 {
            color: #ecf0f1;
        }
        .dark-theme-text .text-gray-500 {
...
<body class="flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-2xl mx-auto">
        <div class="card">
            ...
            <div id="checkInForm" class="p-6 md:p-8 space-y-6 dark-theme-text">
                <div>
                    <label for="name" class="block text-lg font-semibold mb-2">Who we got here? Tell me your name:</label>
                    <input type="text" id="name" name="name" class="dark-input w-full px-4 py-3 rounded-lg transition" placeholder="e.g., Alex Johnson">
                </div>
                
                <div>
                    <label for="morale" class="block text-lg font-semibold mb-2">On a scale of 1 to 10, how you feelin' today?</label>
                    <input type="number" id="morale" name="morale" min="1" max="10" class="dark-input w-full px-4 py-3 rounded-lg transition" placeholder="1 (Low battery) to 10 (Fully charged!)">
                </div>
                
                <div>
                    <label for="understanding" class="block text-lg font-semibold mb-2">And 1 to 10, how's your understanding of the lessons?</label>
                    <input type="number" id="understanding" name="understanding" min="1" max="10" class="dark-input w-full px-4 py-3 rounded-lg transition" placeholder="1 (In the fog) to 10 (Crystal clear)">
                </div>
                ...
                <div class="flex flex-col sm:flex-row gap-4 pt-2">
                    ...
                    <button id="whistleBtn" class="bg-gray-600 hover:bg-gray-700 w-full text-white font-bold py-3 px-6 rounded-lg text-lg transition-colors">End Check-in</button>
                </div>
            </div>

            <div class="dark-theme-text bg-black/20 p-6 md:p-8">
                <h2 class="text-2xl font-bold border-b-2 border-gray-500/50 pb-2 mb-4">Daily Check-in Roster</h2>
    ...
    <script>
        ...
        checkInBtn.addEventListener('click', () => {
            ...
            const morale = parseInt(moraleInput.value);
            const understanding = parseInt(understandingInput.value);

            if (!name || !morale || !understanding) {
                errorMessage.textContent = "C'mon now, gotta fill out all the fields.";
                return;
            }
            if (morale < 1 || morale > 10 || understanding < 1 || understanding > 10) {
                errorMessage.textContent = "Whoa there. Those scores need to be between 1 and 10.";
                return;
            }
            
            errorMessage.textContent = '';
            const capitalizedName = name.charAt(0).toUpperCase() + name.slice(1);
            attendanceLog[capitalizedName] = { morale, understanding };
        ...
        whistleBtn.addEventListener('click', () => {
            document.getElementById('checkInForm').innerHTML = `
                <div class="text-center p-8 dark-theme-text">
                    <h2 class="text-2xl font-bold">Check-in Complete!</h2>
                    <p class="mt-2 text-gray-300">Great work today, team. Remember to take care of yourselves, and each other.</p>
                </div>
            `;
        });
        ...
        function updateRoster() {
            ...
            for (const name in attendanceLog) {
                const scores = attendanceLog[name];
                const playerDiv = document.createElement('div');
                playerDiv.className = 'roster-item p-3 rounded-lg shadow-sm';
                playerDiv.innerHTML = `
                    <p class="font-bold text-lg text-gray-100">${name}</p>
                    <p class="text-sm text-gray-300">Morale: <span class="font-semibold text-gray-100">${scores.morale}/10</span> | Understanding: <span class="font-semibold text-gray-100">${scores.understanding}/10</span></p>
                `;
...
# This defines the main "route" or "page" of our web application.
@app.route('/')
def home():
    return render_template_string(INDEX_HTML)

# This ensures that the web server only runs when the script is executed directly.
